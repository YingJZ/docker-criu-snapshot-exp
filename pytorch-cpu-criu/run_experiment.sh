...
verify_inference() {
    ...
    # 输出验证结果
    info "推理验证结果:"
    docker exec "${CONTAINER_NAME}" cat /app/results/verification.json | python3 -c "
import sys, json
try:
    data = json.loads(sys.stdin.read())
...
report() {
    ...
    echo ""
    echo "============================================================"
    echo "  方案A：PyTorch CPU推理 CRIU 冷启动加速 - 实验报告"
    echo "============================================================"
    echo ""
    echo "  冷启动 (无 CRIU):"
    ...
    echo ""
    echo "  验证状态: $(docker exec "${CONTAINER_NAME}" cat /app/results/verification.json 2>/dev/null | python3 -c "import sys,json; d=json.loads(sys.stdin.read()); print('✅ 通过' if d.get('match') else '❌ 失败')" 2>/dev/null || echo '❓ 未完成')"
    echo "============================================================"
    echo ""
...
main() {
    echo ""
    echo "============================================================"
    echo "  方案A：PyTorch CPU推理 + CRIU 快照加速冷启动"
    echo "============================================================"
    echo ""
